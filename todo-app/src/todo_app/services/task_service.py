...
def keyword_search(keyword: list[str]) -> Union[Tuple[bool, str], Tuple[bool, List[Any]]]:
    """Search todo-app.json by keywords.
        args:
            keyword: list of word to perform the search query by eg. ["meeting", "morning", "tomorrow"]
        return:
    ...
    if not todo_records:
        mssg = f'⛔ No record found - memory is Empty'
        return False, mssg
    ...
    try:
        for task in todo_records:
            if any(search_pattern.search(str(value)) for value in task.values()):
                search_result.append(task)  # store task with keyword to search_result

        
        if not search_result:
            mssg = f'No search result found for {keyword}'  # return an error message is no task has the keyword
            return False, mssg
...
def tag_filter(tag_list: list[str]) -> Union[Tuple[bool, str], Tuple[bool, List[Any]]]:
...
def time_filter(time_list: list[str]) -> Union[Tuple[bool, str], Tuple[bool, List[Any]]]:
...
def priority_filter(priority_list: list[str]) -> Union[Tuple[bool, str], Tuple[bool, List[Any]]]:
